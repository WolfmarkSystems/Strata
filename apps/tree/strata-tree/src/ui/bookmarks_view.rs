...
const TAG_CATEGORIES: &[(&str, &str, egui::Color32)] = &[
    ("\u{1F534}", "Critical Evidence", egui::Color32::from_rgb(0xa8, 0x40, 0x40)),
    ("\u{1F7E0}", "Suspicious",       egui::Color32::from_rgb(0xb8, 0x78, 0x40)),
    ("\u{1F7E1}", "Needs Review",     egui::Color32::from_rgb(0xc8, 0xa0, 0x40)),
    ("\u{1F7E2}", "Confirmed Clean",  egui::Color32::from_rgb(0x48, 0x78, 0x58)),
    ("\u{1F535}", "Key Artifact",     egui::Color32::from_rgb(0x4a, 0x70, 0xc0)),
    ("\u{26AB}",  "Excluded",         egui::Color32::from_rgb(0x3a, 0x48, 0x58)),
];
...
pub fn render(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    ui.horizontal(|ui| {
        ...
        ui.vertical(|ui| {
            ui.set_width(left_w);
            egui::ScrollArea::vertical().id_source("tag_list").show(ui, |ui| {
                // "All" entry
                ...
                // Custom tags
                let custom_tags: Vec<String> = state.bookmarks.iter()
        ...
        ui.vertical(|ui| {
            ...
            let filtered: Vec<_> = if state.active_tag.is_empty() {
                ...
                state.bookmarks.iter().filter(|b| b.tag == state.active_tag).cloned().collect()
            };

            egui::ScrollArea::vertical().id_source("tagged_files").show(ui, |ui| {
                for bm in &filtered {
                    ...
                    // Find tag color
                    let tag_color = TAG_CATEGORIES.iter()
                    ...
                    ui.horizontal(|ui| {
                        ...
                        ui.painter().circle_filled(dot_rect.center(), 4.0, tag_color);

                        let resp = ui.selectable_label(false, egui::RichText::new(&label).color(t.text).size(10.0));
                        ...
                        resp.context_menu(|ui| {
                            if ui.button("Remove Tag").clicked() {
                                ...
                                state.log_action("TAG_REMOVE", &format!("bookmark_id={}", bm.id));
                                ui.close_menu();
                            }
                        });
                    });
                    ...
                    ui.label(
                        egui::RichText::new(format!("{} | {} | {}", bm.tag, bm.note, bm.examiner))
                            .size(8.0)
                            .color(t.muted),
                    );
                    ui.add_space(2.0);
