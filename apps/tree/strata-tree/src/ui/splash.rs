...
pub fn render(ctx: &egui::Context, state: &mut AppState) {
    ...
    // Subtle radial gradient: paint darker edges, lighter center
    egui::CentralPanel::default()
        .frame(
            egui::Frame::default()
                .fill(egui::Color32::from_rgb(0x07, 0x08, 0x09))
                .inner_margin(egui::Margin::same(0.0)),
        )
        .show(ctx, |ui| {
            // Paint subtle lighter center area
            let screen = ui.available_rect_before_wrap();
            let center = screen.center();
            let glow_rect = egui::Rect::from_center_size(center, egui::vec2(600.0, 600.0));
            ui.painter().rect_filled(glow_rect, 300.0, egui::Color32::from_rgb(0x0c, 0x0e, 0x14));

            ui.vertical_centered(|ui| {
                // Push content down to vertically center
                let top_pad = ((screen.height() - 520.0) / 2.0).max(20.0);
                ui.add_space(top_pad);

                // Chevron stack mark — 100x88
                let (chevron_rect, _) =
                    ui.allocate_exact_size(egui::vec2(100.0, 88.0), egui::Sense::hover());
                draw_chevron_stack(ui.painter(), chevron_rect);
                ui.add_space(18.0);

                // STRATA wordmark — 52px
                ui.label(
                ...
                ui.label(
                    egui::RichText::new("Forensic Intelligence Platform")
                        .color(egui::Color32::from_rgb(0x3a, 0x48, 0x58))
                        .size(14.0),
                );
                ...
                egui::Frame::none()
                    ...
                    .show(ui, |ui| {
                        ...
                        // Start Trial button (primary) — 44px height
                        let trial_btn = ui.add_sized(
                            [380.0, 44.0],
                            egui::Button::new(
                                egui::RichText::new("Start Trial")
                                    .color(egui::Color32::from_rgb(0x07, 0x08, 0x09))
                                    .size(14.0)
                                    .strong(),
                            )
                            .fill(egui::Color32::from_rgb(0xd8, 0xe2, 0xec)),
                        );
                        ...
                        // Activate License button (secondary) — 38px
                        let activate_btn = ui.add_sized(
                            [380.0, 38.0],
                            egui::Button::new(
                                egui::RichText::new("Activate License")
                                    .color(egui::Color32::from_rgb(0x8a, 0x9a, 0xaa))
                                    .size(12.0),
                            )
                            .fill(egui::Color32::from_rgb(0x0f, 0x10, 0x14))
                            .stroke(egui::Stroke::new(1.0, egui::Color32::from_rgb(0x18, 0x1c, 0x24))),
                        );
