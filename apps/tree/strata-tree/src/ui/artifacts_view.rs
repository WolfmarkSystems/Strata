...
pub fn render(ui: &mut egui::Ui, state: &mut AppState) {
    ...
    if state.artifact_total == 0 && state.artifact_counts.is_empty() {
        ...
        ui.add_space(8.0);
        ui.label(
            egui::RichText::new("Run artifact plugins to populate categories.")
                .color(t.muted)
                .size(9.0),
        );
        ui.add_space(12.0);
        if ui
            .button(egui::RichText::new("Open Plugins \u{2192}").color(t.active))
            .clicked()
        {
    ...
    ui.horizontal(|ui| {
        ...
        ui.vertical(|ui| {
            ui.set_width(cat_w);
            ui.label(
                egui::RichText::new("ARTIFACT CATEGORIES")
                    .color(t.muted)
                    .size(8.5)
                    .strong(),
            );
        ...
        ui.vertical(|ui| {
            ...
            if let Some(cat) = active_cat {
                let records: Vec<&strata_plugin_sdk::ArtifactRecord> = state
                    ...
                    .filter(|r| {
                        let c = r.category.as_str();
                        c == cat.name
                            || cat
                                .subcategories
                                .iter()
                                .any(|s| r.subcategory == *s || c == *s)
                    })
                    .take(500)
                    .collect();

                ui.label(
                    egui::RichText::new(format!("{} \u{2014} {} results", cat.name, records.len()))
                        .color(t.active)
                        .size(10.0)
                        .strong(),
                );
                    ...
                    .show(ui, |ui| {
                        for (idx, record) in records.iter().enumerate() {
                            ...
                            egui::Frame::none().fill(bg).show(ui, |ui| {
                                ...
                                ui.horizontal(|ui| {
                                    ...
                                    let title_short: String =
                                        record.title.chars().take(40).collect();
                                    ui.label(
                                    ...
                                    ui.add_space(8.0);
                                    let detail_short: String =
                                        record.detail.chars().take(30).collect();
                                    ui.label(
                                        egui::RichText::new(&detail_short).color(t.muted).size(9.0),
                                    );
                                    ui.add_space(8.0);
                                    let ts = record
                                        .timestamp
                            ...
                            let click = ui.interact(
                                row_rect,
                                egui::Id::new(format!("artifact_row_{}", idx)),
                                egui::Sense::click(),
                            );
                            if click.clicked() {
                ...
                ui.vertical_centered(|ui| {
                    ...
                    ui.label(
                        egui::RichText::new("Select a category")
                            .color(t.muted)
                            .size(12.0),
                    );
                });
        ...
        ui.vertical(|ui| {
            ...
            egui::Frame::none()
                ...
                .show(ui, |ui| {
                    ...
                    ui.label(
                        egui::RichText::new("ARTIFACT DETAIL")
                            .color(t.muted)
                            .size(8.5)
                            .strong(),
                    );
                    ...
                    let selected_record = active_cat.and_then(|cat| {
                        let records: Vec<&strata_plugin_sdk::ArtifactRecord> = state
                            ...
                            .collect();
                        state
                            .selected_artifact_idx
                            .and_then(|idx| records.get(idx).copied())
                    });
                    ...
                    if let Some(record) = selected_record {
                        ...
                        egui::ScrollArea::vertical()
                            ...
                            .show(ui, |ui| {
                                ...
                                let (fv_label, fv_color) = match record.forensic_value {
                                    ...
                                    strata_plugin_sdk::ForensicValue::Medium => {
                                        ("Medium", TEXT_SEC)
                                    }
                                    _ => ("Low", TEXT_MUTED),
                                };
                                ui.horizontal(|ui| {
                                    ui.label(
                                        egui::RichText::new("Forensic Value:")
                                            .color(t.muted)
                                            .size(9.0),
                                    );
                                    ui.label(
                                        egui::RichText::new(fv_label)
                                            .color(fv_color)
                                            .size(9.0)
                                            .strong(),
                                    );
                                ...
                                if let Some(raw) = &record.raw_data {
                                    ...
                                    egui::CollapsingHeader::new(
                                        egui::RichText::new("RAW DATA").color(t.muted).size(8.5),
                                    )
                                    .default_open(false)
                                    .show(ui, |ui| {
                                        let json_text =
                        ...
                        ui.vertical_centered(|ui| {
                            ui.add_space(ui.available_height() / 3.0);
                            ui.label(
                                egui::RichText::new("Select an artifact to view details")
                                    .color(t.muted)
                                    .size(12.0),
                            );
                        });
...
fn render_obstruction_gauge(ui: &mut egui::Ui, state: &AppState, t: &crate::theme::StrataTheme) {
