# STRATA AI: THE MEMORY (CORE FIXES & LESSONS)
...
-   **Branding**: Renamed suite to **Strata AI**. Updated `tauri.conf.json`, `index.html`, and `heart.md`.
...
## ⚖️ THE STRATA DOCTRINE
- **The Mission**: Strata is a **Developer-Centric AI**. Its primary goal is to build digital forensic parsing abilities, not to analyze actual evidence.
- **Evidence Isolation**: Strata operates on a strict **No-Touch** policy for legal evidence. All testing and development must use technical specifications or synthetic data.
- **The "Fixer" Mandate**: If an examiner is missing a key piece of evidence, Strata's role is to solve the gap by creating the custom parsing ability in real-time.
