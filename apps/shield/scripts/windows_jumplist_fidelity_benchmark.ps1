...
foreach ($limit in $limits) {
    $runs += Invoke-JumpListBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "jumplist_fidelity_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "JumpList fidelity benchmark summary written to: $summaryPath" -ForegroundColor Green
