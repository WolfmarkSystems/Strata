...
foreach ($limit in $limits) {
    $runs += Invoke-LnkBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "lnk_shortcut_fidelity_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "LNK shortcut fidelity benchmark summary written to: $summaryPath" -ForegroundColor Green
