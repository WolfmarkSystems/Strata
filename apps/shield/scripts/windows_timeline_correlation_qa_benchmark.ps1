...
function Invoke-TimelineCorrelationBench {
    param(
...
foreach ($limit in $limits) {
    $runs += Invoke-TimelineCorrelationBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "timeline_correlation_qa_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "Timeline correlation QA benchmark summary written to: $summaryPath" -ForegroundColor Green
