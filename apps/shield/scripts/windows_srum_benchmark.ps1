...
foreach ($limit in $limits) {
    $runs += Invoke-SrumBench -RepoRoot $repoRoot -InputPath $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "srum_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "SRUM benchmark summary written to: $summaryPath" -ForegroundColor Green
