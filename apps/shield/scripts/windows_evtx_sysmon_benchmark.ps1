...
function Invoke-EvtxSysmonBench {
    param(
...
foreach ($limit in $limits) {
    $runs += Invoke-EvtxSysmonBench -RepoRoot $repoRoot -InputPath $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "evtx_sysmon_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "EVTX sysmon benchmark summary written to: $summaryPath" -ForegroundColor Green
