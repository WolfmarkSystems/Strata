...
function Invoke-NtfsLogFileBench {
    param(
...
foreach ($limit in $limits) {
    $runs += Invoke-NtfsLogFileBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "ntfs_logfile_signals_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "NTFS LogFile signals benchmark summary written to: $summaryPath" -ForegroundColor Green
