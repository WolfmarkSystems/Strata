param(
    [Parameter(Mandatory = $true)][string]$MftInput,
...
function Invoke-NtfsMftFidelityBench {
    param(
...
$resolvedInput = Resolve-Path $MftInput
...
foreach ($limit in $limits) {
    $runs += Invoke-NtfsMftFidelityBench -RepoRoot $repoRoot -MftInput $resolvedInput -Limit $limit
}

$summary = [pscustomobject]@{
    timestamp_utc = [DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fffZ")
    inputs = [ordered]@{
        mft_input = "$resolvedInput"
    }
    runs = $runs
}

$summaryPath = Join-Path $outDir "ntfs_mft_fidelity_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "NTFS MFT fidelity benchmark summary written to: $summaryPath" -ForegroundColor Green
