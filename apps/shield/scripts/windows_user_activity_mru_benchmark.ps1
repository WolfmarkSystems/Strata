...
function Invoke-UserActivityBench {
    param(
...
foreach ($limit in $limits) {
    $runs += Invoke-UserActivityBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "user_activity_mru_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "User-activity/MRU benchmark summary written to: $summaryPath" -ForegroundColor Green
