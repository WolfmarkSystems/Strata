...
foreach ($limit in $limits) {
    $runs += Invoke-UsbBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "usb_device_history_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "USB device history benchmark summary written to: $summaryPath" -ForegroundColor Green
