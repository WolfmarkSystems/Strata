...
function Invoke-RestoreShadowBench {
    param(
...
foreach ($limit in $limits) {
    $runs += Invoke-RestoreShadowBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "restore_shadow_copies_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "Restore/shadow benchmark summary written to: $summaryPath" -ForegroundColor Green
