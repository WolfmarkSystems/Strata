...
foreach ($limit in $limits) {
    $runs += Invoke-BrowserBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "browser_forensics_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "Browser forensics benchmark summary written to: $summaryPath" -ForegroundColor Green
