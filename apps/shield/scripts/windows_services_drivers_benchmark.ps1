param(
    [Parameter(Mandatory = $true)][string]$ServicesRegPath,
...
function Invoke-ServicesDriversBench {
    param(
...
$resolvedInput = Resolve-Path $ServicesRegPath
...
foreach ($limit in $limits) {
    $runs += Invoke-ServicesDriversBench -RepoRoot $repoRoot -ServicesRegPath $resolvedInput -Limit $limit
}

$summary = [pscustomobject]@{
    timestamp_utc = [DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fffZ")
    inputs = [ordered]@{
        services_reg = "$resolvedInput"
    }
    runs = $runs
}

$summaryPath = Join-Path $outDir "services_drivers_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "Services/Drivers benchmark summary written to: $summaryPath" -ForegroundColor Green
