...
foreach ($limit in $limits) {
    $runs += Invoke-BamDamBench -RepoRoot $repoRoot -BamRegPath $resolvedBam -Limit $limit
}
...
$summaryPath = Join-Path $outDir "bam_dam_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "BAM/DAM benchmark summary written to: $summaryPath" -ForegroundColor Green
