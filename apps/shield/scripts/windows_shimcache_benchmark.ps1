param(
    [Parameter(Mandatory = $true)][string]$AppCompatRegPath,
...
function Invoke-ShimcacheBench {
    param(
    ...
    $argList = @(
        ...
        "shimcache-deep",
        "--appcompat-reg", $AppCompatRegPath,
        "--limit", "$Limit",
        "--json",
        "--quiet"
    )
...
$resolvedAppCompat = Resolve-Path $AppCompatRegPath
...
foreach ($limit in $limits) {
    $runs += Invoke-ShimcacheBench -RepoRoot $repoRoot -AppCompatRegPath $resolvedAppCompat -Limit $limit
}

$summary = [pscustomobject]@{
    timestamp_utc = [DateTime]::UtcNow.ToString("yyyy-MM-ddTHH:mm:ss.fffZ")
    inputs = [ordered]@{
        appcompat_reg = "$resolvedAppCompat"
    }
    runs = $runs
}

$summaryPath = Join-Path $outDir "shimcache_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "ShimCache benchmark summary written to: $summaryPath" -ForegroundColor Green
