...
function Invoke-RecycleBinBench {
    param(
...
foreach ($limit in $limits) {
    $runs += Invoke-RecycleBinBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "recycle_bin_artifacts_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "Recycle Bin artifacts benchmark summary written to: $summaryPath" -ForegroundColor Green
