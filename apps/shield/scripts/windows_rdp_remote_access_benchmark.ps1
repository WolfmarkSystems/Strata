...
foreach ($limit in $limits) {
    $runs += Invoke-RdpBench -RepoRoot $repoRoot -Input $resolvedInput -Limit $limit
}
...
$summaryPath = Join-Path $outDir "rdp_remote_access_benchmark_summary.json"
$summary | ConvertTo-Json -Depth 6 | Set-Content -Path $summaryPath -Encoding UTF8
Write-Host "RDP remote access benchmark summary written to: $summaryPath" -ForegroundColor Green
