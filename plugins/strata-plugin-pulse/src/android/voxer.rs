...
mod tests {
    ...
    fn sender_and_thread_in_detail() {
        let db = make_db();
        let r = parse(db.path());
        assert!(r.iter().any(|a| a.detail.contains("sender_id='user_a'") && a.detail.contains("thread_id='thread_1'")));
        // Audio message with no body gets [type] in title
        assert!(r.iter().any(|a| a.title.contains("user_b") && a.title.contains("[audio]")));
    }
