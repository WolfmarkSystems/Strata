    ...
    for (table, label) in [
        ...
        if util::table_exists(&conn, table) {
            ...
            out.push(ArtifactRecord {
                ...
                subcategory: label.to_string(), timestamp: None,
...
mod tests {
    ...
    fn matches_skype_paths() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Skype/main.db")));
    ...
    fn parses_messages() {
        ...
        c.execute("CREATE TABLE Messages (id INTEGER PRIMARY KEY, body TEXT)", []).unwrap();
        c.execute("INSERT INTO Messages (body) VALUES ('hi')", []).unwrap();
