//! iOS Measure app — `com.apple.Measure/` databases.
//! Saved measurements with photos. May contain crime scene measurements.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "com.apple.measure") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Measure".to_string(), timestamp: None,
        title: "iOS Measure app saved measurements".to_string(),
...
mod tests {
    ...
    #[test]
    fn matches_measure() {
        assert!(matches(Path::new("/var/mobile/Library/com.apple.Measure/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("com.apple.Measure");
        std::fs::create_dir_all(&root).unwrap();
        let p = root.join("store.db");
        let c = Connection::open(&p).unwrap();
        c.execute("CREATE TABLE measurements (id INTEGER PRIMARY KEY)", []).unwrap();
        c.execute("INSERT INTO measurements DEFAULT VALUES", []).unwrap();
