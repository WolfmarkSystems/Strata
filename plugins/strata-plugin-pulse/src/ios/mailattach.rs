...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    if !util::table_exists(&conn, "attachments") { return out; }
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Mail attachments".to_string(), timestamp: None,
...
mod tests {
    ...
    fn parses_attachment_count() {
        ...
        c.execute("CREATE TABLE attachments (ROWID INTEGER PRIMARY KEY, name TEXT, mime_type TEXT)", []).unwrap();
        c.execute("INSERT INTO attachments (name, mime_type) VALUES ('doc.pdf', 'application/pdf')", []).unwrap();
        c.execute("INSERT INTO attachments (name, mime_type) VALUES ('img.jpg', 'image/jpeg')", []).unwrap();
        let recs = parse(&p);
    ...
    fn no_attachments_table_returns_empty() {
        ...
        let p = dir.path().join("Envelope Index");
        let c = Connection::open(&p).unwrap();
        c.execute("CREATE TABLE messages (ROWID INTEGER PRIMARY KEY)", []).unwrap();
    ...
    fn empty_attachments_returns_empty() {
        ...
        let p = dir.path().join("Envelope Index");
        let c = Connection::open(&p).unwrap();
        c.execute("CREATE TABLE attachments (ROWID INTEGER PRIMARY KEY)", []).unwrap();
        assert!(parse(&p).is_empty());
    }
}
