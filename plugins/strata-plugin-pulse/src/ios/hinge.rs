//! Hinge iOS — `*.db` / `*.sqlite` under `*hinge*`.
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Hinge".to_string(), timestamp: None,
        title: "Hinge iOS database".to_string(),
...
mod tests {
    ...
    #[test]
    fn matches_hinge() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Hinge/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses_rows() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("hinge");
