...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    for table in ["WifiLocation", "ZRTCLLOCATIONMO", "wifi_locations"] {
        if util::table_exists(&conn, table) {
            ...
            out.push(ArtifactRecord {
                ...
                subcategory: "Wi-Fi geolocation".to_string(), timestamp: None,
                title: format!("iOS Wi-Fi geolocation cache ({})", table),
                detail: format!("{} {} rows — BSSID → lat/lon cache from Apple positioning", count, table),
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!("{} rows across {} tables — Wi-Fi BSSID location cache", total, tables.len()),
...
mod tests {
    ...
    fn matches_wifi_geo_dbs() {
        assert!(matches(Path::new("/var/root/Library/Caches/locationd/cache_encryptedA.db")));
    ...
    fn parses_known_wifi_location_table() {
        ...
        c.execute("CREATE TABLE WifiLocation (MAC TEXT, Latitude REAL, Longitude REAL)", []).unwrap();
        c.execute("INSERT INTO WifiLocation VALUES ('aa:bb:cc:dd:ee:ff', 40.7, -74.0)", []).unwrap();
