...
mod tests {
    ...
    fn matches_netflix_filenames() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/nfcache.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE viewing_history (id INTEGER, title TEXT)", [])
            .unwrap();
        c.execute("INSERT INTO viewing_history VALUES (1, 'Show')", [])
            .unwrap();
