...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    if n.ends_with(".db") || n.ends_with(".sqlite") {
        ...
        return vec![ArtifactRecord {
            ...
            subcategory: "MobileAsset".to_string(),
            timestamp: None,
            title: "iOS MobileAsset OTA update history".to_string(),
            detail: format!(
                "{} rows — OTA downloads: iOS updates, ML models, fonts",
                total
            ),
            source_path: source,
    ...
    vec![ArtifactRecord {
        ...
        detail: format!(
            "MobileAsset plist ({} bytes) — OTA update configuration + download history",
            size
        ),
        source_path: source,
        forensic_value: ForensicValue::High,
...
mod tests {
    ...
    fn parses_db() {
        ...
        .unwrap();
        c.execute("INSERT INTO assets (name) VALUES ('iOS 18.2')", [])
            .unwrap();
