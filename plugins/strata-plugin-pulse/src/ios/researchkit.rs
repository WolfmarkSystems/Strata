...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "ResearchKit".to_string(), timestamp: None,
        title: "iOS ResearchKit study data".to_string(),
        detail: format!("{} rows — study enrollment, surveys, sensor data, consent records", total),
...
mod tests {
    ...
    fn matches_researchkit() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/ResearchKit/study.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE surveys (id INTEGER PRIMARY KEY, result TEXT)", []).unwrap();
        c.execute("INSERT INTO surveys (result) VALUES ('completed')", []).unwrap();
