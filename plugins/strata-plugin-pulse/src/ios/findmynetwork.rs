...
pub fn matches(path: &Path) -> bool {
    let in_findmy = util::path_contains(path, "searchpartyd")
        || util::path_contains(path, "com.apple.findmy");
    in_findmy && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Find My network".to_string(), timestamp: None,
...
mod tests {
    ...
    fn matches_findmy_dbs() {
        assert!(matches(Path::new("/var/mobile/Library/com.apple.icloud.searchpartyd/FindMyCache.db")));
        assert!(matches(Path::new("/var/mobile/Library/com.apple.findmy/items.sqlite")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE item_locations (id INTEGER PRIMARY KEY, lat REAL, lon REAL, ts REAL)", []).unwrap();
        c.execute("INSERT INTO item_locations (lat, lon, ts) VALUES (40.7, -74.0, 700000000.0)", []).unwrap();
        let recs = parse(&p);
