...
pub fn matches(path: &Path) -> bool {
    if !util::path_contains(path, "robinhood") {
        return false;
    }
    let n = path
        .file_name()
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!(
            "{} rows across {} tables — portfolio, trades, watchlist",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_robinhood() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/Robinhood/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE trades (id INTEGER PRIMARY KEY, symbol TEXT, amount REAL)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO trades (symbol, amount) VALUES ('GME', 100.0)",
            [],
        )
        .unwrap();
        let recs = parse(&p);
