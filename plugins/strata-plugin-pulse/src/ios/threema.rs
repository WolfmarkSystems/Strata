...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    if util::table_exists(&conn, "ZMESSAGE") {
        ...
        out.push(ArtifactRecord {
            ...
            subcategory: "Threema messages".to_string(),
            timestamp: None,
    ...
    if util::table_exists(&conn, "ZCONTACT") {
        ...
        out.push(ArtifactRecord {
            category: ArtifactCategory::Communications,
            subcategory: "Threema contacts".to_string(),
            timestamp: None,
...
mod tests {
    ...
    fn matches_threema() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/ThreemaData.sqlite"
        )));
    ...
    fn parses_messages_and_contacts() {
        ...
        .unwrap();
        c.execute(
            "CREATE TABLE ZCONTACT (Z_PK INTEGER PRIMARY KEY, ZNAME TEXT)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO ZMESSAGE (ZTEXT) VALUES ('hi')", [])
            .unwrap();
        c.execute("INSERT INTO ZCONTACT (ZNAME) VALUES ('Alice')", [])
            .unwrap();
