...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
        ...
        if let Some(conn) = util::open_sqlite_ro(path) {
            ...
            if !tables.is_empty() {
                ...
                return vec![ArtifactRecord {
                    ...
                    subcategory: "MediaRemote".to_string(),
                    timestamp: None,
                    title: "iOS now-playing / media playback history".to_string(),
                    detail: format!(
                        "{} rows — song/podcast/video titles, artists, playback times",
                        total
                    ),
                    source_path: source,
    ...
    vec![ArtifactRecord {
        ...
        detail: format!(
            "MediaRemote data ({} bytes) — recently played media metadata",
            size
        ),
        source_path: source,
        forensic_value: ForensicValue::High,
...
mod tests {
    ...
    fn matches_mediaremote() {
        assert!(matches(Path::new(
            "/var/mobile/Library/MediaRemote/store.db"
        )));
