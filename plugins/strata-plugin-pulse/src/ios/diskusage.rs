...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Disk usage".to_string(),
        timestamp: None,
        title: "iOS storage management / disk usage".to_string(),
        detail: format!(
            "{} rows — per-app storage consumption, cache deletion, offloading decisions",
            total
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_cachedelete() {
        assert!(matches(Path::new(
            "/var/mobile/Library/CacheDelete/CacheDeleteAppCache.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE app_cache (id INTEGER PRIMARY KEY, bundle TEXT)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO app_cache (bundle) VALUES ('com.app')", [])
            .unwrap();
