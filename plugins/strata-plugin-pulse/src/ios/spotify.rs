...
pub fn matches(path: &Path) -> bool {
    if !util::path_contains(path, "spotify") {
        return false;
    }
    let n = path
        .file_name()
        ...
        .to_ascii_lowercase();
    n == "offline.bnk"
        || n == "persistent_cache.db"
        || n == "recently_played.db"
        || n.ends_with(".db")
        || n.ends_with(".sqlite")
}
...
mod tests {
    ...
    fn matches_spotify_files() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/Caches/Spotify/offline.bnk"
        )));
