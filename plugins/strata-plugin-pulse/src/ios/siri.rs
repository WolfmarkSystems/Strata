...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Siri".to_string(),
        timestamp: None,
        title: "iOS Siri interactions".to_string(),
        detail: format!(
            "{} rows across {} tables — voice queries, responses, learned suggestions",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_siri() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Assistant/assistant.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE interactions (id INTEGER PRIMARY KEY, query TEXT)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO interactions (query) VALUES ('What time is it')",
            [],
        )
        .unwrap();
