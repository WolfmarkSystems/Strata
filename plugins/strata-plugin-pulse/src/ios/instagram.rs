...
mod tests {
    ...
    fn matches_instagram_filenames() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/Caches/DirectPhotos.sqlite"
        )));
    ...
    fn parses_table_count() {
        ...
        c.execute("CREATE TABLE photos (id INTEGER PRIMARY KEY, url TEXT)", [])
            .unwrap();
        c.execute(
            "INSERT INTO photos (url) VALUES ('https://ig.com/a.jpg')",
            [],
        )
        .unwrap();
        let recs = parse(&p);
