...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "iCloud Key-Value".to_string(),
        timestamp: None,
        title: "iOS iCloud Key-Value store".to_string(),
        detail: format!(
            "{} rows — per-app synced settings (survives reinstall)",
            total
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_kvs() {
        assert!(matches(Path::new(
            "/var/mobile/Library/com.apple.kvs/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE kvs (key TEXT, value BLOB)", [])
            .unwrap();
        c.execute("INSERT INTO kvs VALUES ('k', x'FF')", [])
            .unwrap();
