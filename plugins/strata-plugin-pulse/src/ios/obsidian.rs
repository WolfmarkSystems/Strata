//! iOS Obsidian — markdown notes vault parser.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "obsidian") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        title: "Obsidian iOS database".to_string(),
        detail: format!("{} rows — vault metadata, note titles, graph connections", total),
...
mod tests {
    ...
    #[test]
    fn matches_app() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/obsidian/store.db")));
