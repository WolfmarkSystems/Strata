...
pub fn matches(path: &Path) -> bool {
    ...
    wechat
        && (util::name_is(path, &["mm.sqlite", "wcdb_contact.sqlite", "enmicromsg.db"]) || {
            let n = path
                .file_name()
                .and_then(|n| n.to_str())
                .unwrap_or("")
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    for t in &chat_tables {
        chat_rows += util::count_rows(&conn, t);
    }
    ...
    let detail = if !chat_tables.is_empty() {
        format!(
            "{} Chat_* message rows across {} chats, {} total rows in {} tables",
            chat_rows,
            chat_tables.len(),
            total,
            tables.len()
...
mod tests {
    ...
    fn matches_wechat_paths() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/WeChatPrivate/xxx/MM.sqlite"
        )));
    ...
    fn parses_chat_tables() {
        ...
        c.execute(
            "CREATE TABLE Chat_abc (MesLocalID INTEGER, Message TEXT, CreateTime INTEGER)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO Chat_abc VALUES (1, 'hello', 1700000000)", [])
            .unwrap();
        c.execute("INSERT INTO Chat_abc VALUES (2, 'hi', 1700000001)", [])
            .unwrap();
        let recs = parse(&p);
