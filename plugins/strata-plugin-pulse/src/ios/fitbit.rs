...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Fitbit".to_string(),
        timestamp: None,
        title: "Fitbit iOS database".to_string(),
        detail: format!(
            "{} rows across {} tables — activity, sleep, heart rate",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_fitbit() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/Fitbit/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE activity (id INTEGER PRIMARY KEY, steps INTEGER)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO activity (steps) VALUES (10000)", [])
            .unwrap();
