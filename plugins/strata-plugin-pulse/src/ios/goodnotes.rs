//! iOS GoodNotes — handwritten notes parser.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "goodnotes") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        title: "GoodNotes iOS database".to_string(),
        detail: format!("{} rows — notebooks, pages, handwriting metadata (strokes not extracted)", total),
...
mod tests {
    ...
    #[test]
    fn matches_app() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/goodnotes/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("goodnotes");
