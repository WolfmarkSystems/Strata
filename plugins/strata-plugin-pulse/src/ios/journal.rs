...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Journal".to_string(),
        timestamp: None,
        title: "Apple Journal diary entries".to_string(),
        detail: format!(
            "{} rows — diary entries with photos, locations, workouts, mood (metadata only)",
            total
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_journal() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/com.apple.journal/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE entries (id INTEGER PRIMARY KEY, text TEXT, date DOUBLE)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO entries (text, date) VALUES ('Today was interesting', 700000000.0)",
            [],
        )
        .unwrap();
        let recs = parse(&p);
