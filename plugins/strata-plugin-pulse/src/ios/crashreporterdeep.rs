...
pub fn matches(path: &Path) -> bool {
    ...
    in_analytics
        && util::name_is(
            path,
            &["analytics.db", "diagnosticmessages.db", "crashes.db"],
        )
}
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "CrashReporter analytics".to_string(),
        timestamp: None,
        title: "iOS CrashReporter analytics store".to_string(),
        detail: format!(
            "{} rows across {} tables — aggregated crash counts, jetsam, exception types",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_analytics_db() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Logs/CrashReporter/Analytics/analytics.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE crash_events (id INTEGER PRIMARY KEY, bundle TEXT, count INTEGER)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO crash_events (bundle, count) VALUES ('com.example', 3)",
            [],
        )
        .unwrap();
        let recs = parse(&p);
