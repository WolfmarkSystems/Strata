//! Pinterest iOS — `*.db` under `*pinterest*`.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "pinterest") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Pinterest".to_string(), timestamp: None,
        title: "Pinterest iOS database".to_string(),
        detail: format!("{} rows across {} tables — pins, boards, searches", total, tables.len()),
...
mod tests {
    ...
    #[test]
    fn matches_pinterest() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Pinterest/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses_rows() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("pinterest");
        std::fs::create_dir_all(&root).unwrap();
        let p = root.join("store.db");
        let c = Connection::open(&p).unwrap();
        c.execute("CREATE TABLE pins (id INTEGER PRIMARY KEY)", []).unwrap();
        c.execute("INSERT INTO pins DEFAULT VALUES", []).unwrap();
