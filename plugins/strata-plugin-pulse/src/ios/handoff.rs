...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Handoff".to_string(), timestamp: None,
        title: "iOS Handoff / Continuity activity log".to_string(),
        detail: format!("{} rows — cross-device activity handoffs, reveals associated Apple devices", total),
...
mod tests {
    ...
    #[test]
    fn matches_handoff() {
        assert!(matches(Path::new("/var/mobile/Library/useractivityd/store.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE activities (id INTEGER PRIMARY KEY)", []).unwrap();
        c.execute("INSERT INTO activities DEFAULT VALUES", []).unwrap();
