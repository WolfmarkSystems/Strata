...
pub fn matches(path: &Path) -> bool {
    let n = path
        ...
        .unwrap_or("")
        .to_ascii_lowercase();
    n.starts_with("index.spotlightv") || n == ".store.db" && util::path_contains(path, "spotlight")
}
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    vec![ArtifactRecord {
        ...
        detail: format!(
            "Spotlight index at {} ({} bytes) — searchable metadata for all apps",
            source, size
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_spotlight_index() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Spotlight/index.spotlightV3"
        )));
        assert!(matches(Path::new(
            "/var/mobile/Library/Spotlight/index.spotlightV2"
        )));
