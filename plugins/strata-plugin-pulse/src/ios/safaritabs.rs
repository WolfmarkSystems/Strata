...
mod tests {
    ...
    fn matches_browserstate_in_safari() {
        assert!(matches(Path::new("/var/mobile/Library/Safari/BrowserState.db")));
        assert!(!matches(Path::new("/var/mobile/Library/Other/BrowserState.db")));
    }
    ...
    fn parses_tabs_table() {
        ...
        c.execute("CREATE TABLE tabs (id INTEGER PRIMARY KEY, url TEXT, title TEXT)", []).unwrap();
        c.execute("INSERT INTO tabs (url, title) VALUES ('https://a.com', 'A')", []).unwrap();
        let recs = parse(&p);
