...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "CoreSpotlight index".to_string(),
        timestamp: None,
        title: "iOS CoreSpotlight per-app search index".to_string(),
        detail: format!(
            "{} rows — searchable metadata (may retain deleted content titles/previews)",
            total
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_corespotlight() {
        assert!(matches(Path::new(
            "/var/mobile/Library/CoreSpotlight/NSFileProtectionComplete/index.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE searchable_items (id TEXT PRIMARY KEY, title TEXT)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO searchable_items VALUES ('i1', 'My Document')",
            [],
        )
        .unwrap();
