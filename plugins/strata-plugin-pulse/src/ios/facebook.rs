...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    let detail = if msg_count > 0 {
        format!("{} messages rows, {} total rows across {} tables", msg_count, total, tables.len())
    } else {
...
mod tests {
    ...
    fn matches_messenger_filenames() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Documents/lightspeed-abc123.db")));
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/threads_db2")));
    ...
    fn parses_messages_table_when_present() {
        ...
        c.execute("INSERT INTO messages (text) VALUES ('hello')", []).unwrap();
        c.execute("INSERT INTO messages (text) VALUES ('world')", []).unwrap();
        let recs = parse(&p);
        let r = recs.iter().find(|r| r.subcategory == "Facebook Messenger").unwrap();
