...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!("{} rows across {} tables — iCloud sharing, collaboration records", total, tables.len()),
        source_path: source,
...
mod tests {
    ...
    fn matches_cloudkit_paths() {
        assert!(matches(Path::new("/var/mobile/Library/Caches/CloudKit/cloudkit_cache.sqlite")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE cached_records (id TEXT, recordType TEXT)", []).unwrap();
        c.execute("INSERT INTO cached_records VALUES ('r1', 'Note')", []).unwrap();
        let recs = parse(&p);
