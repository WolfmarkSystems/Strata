//! iOS Wordle/NYT Games — game history parser.
...
pub fn matches(path: &Path) -> bool {
    (util::path_contains(path, "nytimes") || util::path_contains(path, "wordle")) && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        title: "Wordle / NYT Games iOS database".to_string(),
...
mod tests {
    ...
    #[test]
    fn matches_app() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/nytimes/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("nytimes");
