...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Background tasks".to_string(), timestamp: None,
...
mod tests {
    ...
    #[test]
    fn matches_bg_tasks() {
        assert!(matches(Path::new("/var/mobile/Library/BackgroundTasks/store.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE tasks (id INTEGER PRIMARY KEY, bundle TEXT)", []).unwrap();
        c.execute("INSERT INTO tasks (bundle) VALUES ('com.example')", []).unwrap();
