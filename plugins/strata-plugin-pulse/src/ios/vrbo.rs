//! iOS VRBO — vacation rental bookings parser.
...
pub fn matches(path: &Path) -> bool {
    (util::path_contains(path, "vrbo") || util::path_contains(path, "homeaway")) && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        title: "VRBO iOS database".to_string(),
        detail: format!("{} rows — bookings, saved properties, search history", total),
...
mod tests {
    ...
    #[test]
    fn matches_app() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/vrbo/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("vrbo");
