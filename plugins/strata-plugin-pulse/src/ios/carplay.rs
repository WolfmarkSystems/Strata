...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
        ...
        if let Some(conn) = util::open_sqlite_ro(path) {
            ...
            if !tables.is_empty() {
                ...
                return vec![ArtifactRecord {
                    ...
                    subcategory: "CarPlay".to_string(),
                    timestamp: None,
                    title: "iOS CarPlay database".to_string(),
                    detail: format!(
                        "{} rows across {} tables — vehicle pairing, driving session app usage",
                        total,
                        tables.len()
                    ),
                    source_path: source,
    ...
    vec![ArtifactRecord {
        ...
        detail: format!(
            "CarPlay plist ({} bytes) — paired vehicles, app layout, driving sessions",
            size
        ),
        source_path: source,
        forensic_value: ForensicValue::High,
...
mod tests {
    ...
    fn matches_carplay_files() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Preferences/com.apple.carplay.plist"
        )));
