//! Wells Fargo iOS — `*.db` under `*wellsfargo*`.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "wellsfargo") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Wells Fargo".to_string(), timestamp: None,
        title: "Wells Fargo iOS banking database".to_string(),
        detail: format!("{} rows — account info, transactions, transfers", total),
...
mod tests {
    ...
    #[test]
    fn matches_wellsfargo() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/WellsFargo/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("wellsfargo");
        std::fs::create_dir_all(&root).unwrap();
        let p = root.join("store.db");
        let c = Connection::open(&p).unwrap();
        c.execute("CREATE TABLE txns (id INTEGER PRIMARY KEY)", []).unwrap();
        c.execute("INSERT INTO txns DEFAULT VALUES", []).unwrap();
