...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "CloudKit sync state".to_string(),
        timestamp: None,
        title: "iOS CloudKit sync state".to_string(),
        detail: format!(
            "{} rows — record-level iCloud sync tracking, reveals what was pushed to cloud",
            total
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_cloudkit_sync() {
        assert!(matches(Path::new(
            "/var/mobile/Library/CloudKit/CloudKitSyncState.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE sync_state (id TEXT PRIMARY KEY, modified DOUBLE)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO sync_state VALUES ('r1', 700000000.0)", [])
            .unwrap();
