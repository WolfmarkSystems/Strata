...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
        ...
        if let Some(conn) = util::open_sqlite_ro(path) {
            ...
            if !tables.is_empty() {
                ...
                return vec![ArtifactRecord {
                    ...
                    subcategory: "Communication limits".to_string(),
                    timestamp: None,
    ...
    vec![ArtifactRecord {
        ...
        detail: format!(
            "{} ({} bytes) — Screen Time allowed/blocked contacts for child accounts",
            name, size
        ),
        source_path: source,
        forensic_value: ForensicValue::High,
...
mod tests {
    ...
    fn matches_comm_limits() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Preferences/com.apple.screentime.communicationlimits.plist"
        )));
    ...
    fn parses_plist() {
        ...
        let p = dir
            .path()
            .join("com.apple.screentime.communicationlimits.plist");
