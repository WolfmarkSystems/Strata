...
pub fn matches(path: &Path) -> bool {
    if !util::path_contains(path, "coinbase") { return false; }
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!("{} rows across {} tables — crypto wallets, transactions, portfolio", total, tables.len()),
        source_path: source,
...
mod tests {
    ...
    #[test]
    fn matches_coinbase() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Coinbase/store.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE wallets (id TEXT PRIMARY KEY, address TEXT)", []).unwrap();
        c.execute("INSERT INTO wallets VALUES ('w1', '0xdead')", []).unwrap();
