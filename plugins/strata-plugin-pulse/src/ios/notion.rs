//! iOS Notion — notes, databases, workspace parser.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "notion") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        title: "Notion iOS database".to_string(),
        detail: format!("{} rows — pages, databases, workspace content, shared docs", total),
...
mod tests {
    ...
    #[test]
    fn matches_app() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/notion/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("notion");
