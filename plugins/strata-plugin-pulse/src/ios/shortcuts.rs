    ...
    for (table, label) in [
        ...
        if util::table_exists(&conn, table) {
            ...
            out.push(ArtifactRecord {
                ...
                detail: format!("{} {} rows — user-created automations and downloaded shortcuts", count, table),
...
mod tests {
    ...
    fn matches_shortcuts_db() {
        assert!(matches(Path::new("/var/mobile/Library/Shortcuts/Shortcuts.sqlite")));
    ...
    fn parses_shortcut_count() {
        ...
        c.execute("CREATE TABLE ZSHORTCUT (Z_PK INTEGER PRIMARY KEY, ZNAME TEXT, ZCREATIONDATE DOUBLE)", []).unwrap();
        c.execute("INSERT INTO ZSHORTCUT (ZNAME, ZCREATIONDATE) VALUES ('Morning Routine', 700000000.0)", []).unwrap();
        c.execute("INSERT INTO ZSHORTCUT (ZNAME, ZCREATIONDATE) VALUES ('Driving Mode', 700000100.0)", []).unwrap();
        let recs = parse(tmp.path());
        let r = recs.iter().find(|r| r.subcategory.contains("ZSHORTCUT")).unwrap();
