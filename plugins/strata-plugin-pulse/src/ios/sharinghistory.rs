...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Share Sheet history".to_string(), timestamp: None,
        title: "iOS Share Sheet / sharing history".to_string(),
        detail: format!("{} rows — share sheet invocations (which apps content was shared to)", total),
        source_path: source, forensic_value: ForensicValue::High,
        mitre_technique: Some("T1048".to_string()), is_suspicious: false, raw_data: None,
        confidence: 0,
    });
    out
}
...
mod tests {
    ...
    #[test]
    fn matches_sharing() {
        assert!(matches(Path::new("/var/mobile/Library/com.apple.Sharing/store.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE shares (id INTEGER PRIMARY KEY, app TEXT)", []).unwrap();
        c.execute("INSERT INTO shares (app) VALUES ('com.whatsapp')", []).unwrap();
