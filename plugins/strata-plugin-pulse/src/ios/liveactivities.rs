...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Live Activities".to_string(),
        timestamp: None,
        title: "iOS Live Activities / Dynamic Island".to_string(),
        detail: format!(
            "{} rows — real-time activity sessions (ride tracking, deliveries, timers)",
            total
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_activitykit() {
        assert!(matches(Path::new(
            "/var/mobile/Library/ActivityKit/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE activities (id INTEGER PRIMARY KEY, bundle TEXT)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO activities (bundle) VALUES ('com.uber')", [])
            .unwrap();
