//! iOS Bear notes — markdown notes parser.
...
pub fn matches(path: &Path) -> bool {
    (util::path_contains(path, "bear") || util::path_contains(path, "shinyfrog")) && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        title: "Bear Notes iOS database".to_string(),
        detail: format!("{} rows — note titles, tags, creation dates, sync state", total),
...
mod tests {
    ...
    #[test]
    fn matches_app() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/shinyfrog/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("shinyfrog");
