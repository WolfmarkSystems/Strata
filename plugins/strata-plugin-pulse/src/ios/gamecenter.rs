...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    for (table, label) in [
        ...
        if util::table_exists(&conn, table) {
            ...
            out.push(ArtifactRecord {
                ...
                subcategory: format!("Game Center {}", table),
                timestamp: None,
    ...
    if out.is_empty() {
        ...
        out.push(ArtifactRecord {
            category: ArtifactCategory::SocialMedia,
            subcategory: "Game Center".to_string(),
            timestamp: None,
            title: "iOS Game Center database".to_string(),
            detail: format!(
                "{} rows across {} tables — friends, achievements, matches",
                total,
                tables.len()
            ),
            source_path: source,
...
mod tests {
    ...
    fn matches_gamecenter() {
        assert!(matches(Path::new(
            "/var/mobile/Library/GameCenter/GKDatabase.db"
        )));
    ...
    fn parses_friends_table() {
        ...
        c.execute(
            "CREATE TABLE ZFRIEND (Z_PK INTEGER PRIMARY KEY, ZDISPLAYNAME TEXT)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO ZFRIEND (ZDISPLAYNAME) VALUES ('Player1')", [])
            .unwrap();
