...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!(
            "{} rows across {} tables — search history, offline maps",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn parses_row_count() {
        ...
        c.execute("CREATE TABLE recents (q TEXT, ts INTEGER)", [])
            .unwrap();
        c.execute("INSERT INTO recents VALUES ('123 main st', 1700000000)", [])
            .unwrap();
        let recs = parse(&p);
