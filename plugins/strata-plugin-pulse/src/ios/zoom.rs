...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Zoom".to_string(), timestamp: None,
        title: "Zoom iOS database".to_string(),
...
mod tests {
    ...
    #[test]
    fn matches_zoom() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Zoom/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses_rows() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("zoom");
        std::fs::create_dir_all(&root).unwrap();
        let p = root.join("store.db");
        let c = Connection::open(&p).unwrap();
        c.execute("CREATE TABLE meetings (id INTEGER PRIMARY KEY)", []).unwrap();
        c.execute("INSERT INTO meetings DEFAULT VALUES", []).unwrap();
