...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!("{} rows across {} tables — player profile, clan, in-game chat", total, tables.len()),
        source_path: source,
...
mod tests {
    ...
    fn matches_supercell_paths() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/supercell/store.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE chat (id INTEGER PRIMARY KEY, msg TEXT)", []).unwrap();
        c.execute("INSERT INTO chat (msg) VALUES ('attack now')", []).unwrap();
