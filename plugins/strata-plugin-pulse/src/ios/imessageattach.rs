...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    if !util::table_exists(&conn, "message_attachment_join") {
        return out;
    }
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!(
            "{} linked attachments — MIME breakdown: {}",
            total, breakdown
        ),
        source_path: source,
...
mod tests {
    ...
    fn make_sms_with_attachments(rows: &[&str]) -> NamedTempFile {
        ...
        for (i, mime) in rows.iter().enumerate() {
            let aid = (i + 1) as i64;
            c.execute("INSERT INTO message (text) VALUES ('msg')", [])
                .unwrap();
    ...
    fn parses_mime_breakdown() {
        ...
        let r = recs
            .iter()
            .find(|r| r.subcategory == "iMessage attachments")
            .unwrap();
