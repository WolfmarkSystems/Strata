...
pub fn matches(path: &Path) -> bool {
    let in_loc =
        util::path_contains(path, "locationd") || util::path_contains(path, "corelocation");
    in_loc && {
        let n = path
            .file_name()
            .and_then(|n| n.to_str())
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    for (table, label) in [
        (
            "ZRTVISITMO",
            "location visits (arrival/departure with lat/lon)",
        ),
        ...
        if util::table_exists(&conn, table) {
            ...
            out.push(ArtifactRecord {
                ...
                subcategory: format!("CoreLocation {}", table),
                timestamp: None,
...
mod tests {
    ...
    fn matches_locationd_dbs() {
        assert!(matches(Path::new(
            "/var/root/Library/Caches/locationd/cache_encryptedC.db"
        )));
    ...
    fn parses_visit_table() {
        ...
        c.execute(
            "CREATE TABLE ZRTVISITMO (Z_PK INTEGER PRIMARY KEY, ZLATITUDE REAL, ZLONGITUDE REAL)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO ZRTVISITMO (ZLATITUDE, ZLONGITUDE) VALUES (40.7, -74.0)",
            [],
        )
        .unwrap();
        let recs = parse(&p);
