...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!("{} rows across {} tables — payments, contacts", total, tables.len()),
        source_path: source,
...
mod tests {
    ...
    fn matches_cashapp_paths() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/com.squareup.cash/data.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE payments (id INTEGER PRIMARY KEY, amount REAL)", []).unwrap();
        c.execute("INSERT INTO payments (amount) VALUES (25.00)", []).unwrap();
        let recs = parse(&p);
