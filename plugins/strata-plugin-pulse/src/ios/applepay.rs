...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    for (table, label, fv) in [
        ("payment_transaction", "Apple Pay transactions", ForensicValue::Critical),
        ("payment_pass", "Apple Pay cards (DPAN only)", ForensicValue::Critical),
        ("felica_transaction", "Suica/transit transactions", ForensicValue::High),
    ] {
    ...
    if out.is_empty() {
        ...
        out.push(ArtifactRecord {
            category: ArtifactCategory::AccountsCredentials,
            subcategory: "Apple Pay".to_string(), timestamp: None,
...
mod tests {
    ...
    #[test]
    fn matches_passkit() {
        assert!(matches(Path::new("/var/mobile/Library/NanoPassKit/store.sqlite3")));
    ...
    fn parses_payment_transactions() {
        ...
        let recs = parse(&p);
        let t = recs.iter().find(|r| r.subcategory.contains("payment_transaction")).unwrap();
