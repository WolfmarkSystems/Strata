//! Expedia iOS — `*.db` under `*expedia*`.
//!
//! Travel booking: flights, hotels, car rentals with dates and
//! destinations. Proves travel planning and itineraries.
...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "expedia") && {
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Expedia".to_string(), timestamp: None,
        title: "Expedia iOS database".to_string(),
        detail: format!("{} rows — flights, hotels, car rentals, itineraries", total),
...
mod tests {
    ...
    #[test]
    fn matches_expedia() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Expedia/store.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses_rows() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("expedia");
