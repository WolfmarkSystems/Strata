...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "LinkedIn".to_string(),
        timestamp: None,
        title: "LinkedIn iOS database".to_string(),
        detail: format!(
            "{} rows across {} tables — connections, messages",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_linkedin() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/LinkedIn/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE connections (id INTEGER PRIMARY KEY)", [])
            .unwrap();
        c.execute("INSERT INTO connections DEFAULT VALUES", [])
            .unwrap();
