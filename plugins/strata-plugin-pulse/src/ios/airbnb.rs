...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Airbnb".to_string(), timestamp: None,
        title: "Airbnb iOS database".to_string(),
        detail: format!("{} rows — bookings, saved listings, search history, messages", total),
...
mod tests {
    ...
    #[test]
    fn matches_airbnb() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/airbnb/cache.db")));
    ...
    fn parses_rows() {
        ...
        c.execute("CREATE TABLE bookings (id INTEGER PRIMARY KEY)", []).unwrap();
        c.execute("INSERT INTO bookings DEFAULT VALUES", []).unwrap();
