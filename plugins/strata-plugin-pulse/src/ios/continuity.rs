...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    vec![ArtifactRecord {
        ...
        subcategory: "Continuity".to_string(),
        timestamp: None,
        title: format!("iOS Continuity / Sidecar: {}", name),
        detail: format!(
            "{} ({} bytes) — paired Mac/iPad for Continuity Camera, Sidecar, Universal Control",
            name, size
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_continuity() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Preferences/com.apple.sidecar.plist"
        )));
        assert!(matches(Path::new(
            "/var/mobile/Library/Preferences/com.apple.continuity.plist"
        )));
