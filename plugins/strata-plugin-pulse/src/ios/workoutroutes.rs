...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    if n.ends_with(".gpx") || n.ends_with(".route") {
        ...
        return vec![ArtifactRecord {
            ...
            subcategory: "Workout route".to_string(),
            timestamp: None,
            title: "HealthKit workout GPS route".to_string(),
            detail: format!(
                "Workout route file ({} bytes) — full GPS polyline with lat/lon/alt per second",
                size
            ),
            source_path: source,
    ...
    if !util::table_exists(&conn, "workout_events") {
        return out;
    }
    ...
    out.push(ArtifactRecord {
        category: ArtifactCategory::UserActivity,
        subcategory: "Workout routes".to_string(),
        timestamp: None,
        title: "HealthKit workout route events".to_string(),
        detail: format!(
            "{} workout_events rows — GPS route segments from Apple Watch workouts",
            count
        ),
        source_path: source,
        forensic_value: ForensicValue::Critical,
...
mod tests {
    ...
    fn matches_gpx_under_workout_path() {
        assert!(matches(Path::new(
            "/var/mobile/Library/Health/workout_routes/route_001.gpx"
        )));
        assert!(matches(Path::new(
    ...
    fn parses_workout_events_count() {
        ...
        c.execute(
            "CREATE TABLE workout_events (data_id INTEGER PRIMARY KEY, type INTEGER, date DOUBLE)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO workout_events (type, date) VALUES (6, 700000000.0)",
            [],
        )
        .unwrap();
        c.execute(
            "INSERT INTO workout_events (type, date) VALUES (6, 700000001.0)",
            [],
        )
        .unwrap();
    ...
    fn parses_gpx_presence() {
        ...
        std::fs::write(
            &p,
            b"<gpx><trk><trkseg><trkpt lat='40.7' lon='-74.0'/></trkseg></trk></gpx>",
        )
        .unwrap();
        let recs = parse(&p);
        assert_eq!(recs.len(), 1);
    ...
    fn empty_workout_events_returns_empty() {
        ...
        let c = Connection::open(tmp.path()).unwrap();
        c.execute(
            "CREATE TABLE workout_events (data_id INTEGER PRIMARY KEY)",
            [],
        )
        .unwrap();
