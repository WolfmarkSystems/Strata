        ...
        if let Some(conn) = util::open_sqlite_ro(path) {
            ...
            if !tables.is_empty() {
                ...
                return vec![ArtifactRecord {
                    ...
                    subcategory: "Fall Detection".to_string(), timestamp: None,
...
mod tests {
    ...
    fn matches_fall_detection() {
        assert!(matches(Path::new("/var/mobile/Library/WatchSafetyService/events.db")));
        assert!(matches(Path::new("/var/mobile/Library/FallDetection/config.plist")));
