...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Hulu".to_string(), timestamp: None,
        title: "Hulu iOS database".to_string(),
...
mod tests {
    ...
    #[test]
    fn matches_hulu() {
        assert!(matches(Path::new("/var/mobile/Containers/Data/Application/UUID/Library/Hulu/cache.db")));
        assert!(!matches(Path::new("/var/mobile/Library/SMS/sms.db")));
    }
    #[test]
    fn parses_rows() {
        let dir = tempdir().unwrap();
        let root = dir.path().join("hulu");
