...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        subcategory: "Strava".to_string(),
        timestamp: None,
        title: "Strava iOS database".to_string(),
        detail: format!(
            "{} rows across {} tables — GPS-tracked workouts with route geometry",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_strava() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/Strava/store.db"
        )));
    ...
    fn parses_rows() {
        ...
        c.execute(
            "CREATE TABLE activities (id INTEGER PRIMARY KEY, lat REAL, lon REAL)",
            [],
        )
        .unwrap();
        c.execute("INSERT INTO activities (lat, lon) VALUES (40.7, -74.0)", [])
            .unwrap();
        let recs = parse(&p);
