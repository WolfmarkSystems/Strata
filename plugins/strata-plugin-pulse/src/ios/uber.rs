...
pub fn matches(path: &Path) -> bool {
    util::path_contains(path, "uber") && util::name_is(path, &["store.db", "cache.db", "uber.db"])
}
...
pub fn parse(path: &Path) -> Vec<ArtifactRecord> {
    ...
    out.push(ArtifactRecord {
        ...
        detail: format!(
            "{} rows across {} tables — trip history, locations",
            total,
            tables.len()
        ),
        source_path: source,
...
mod tests {
    ...
    fn matches_uber_paths() {
        assert!(matches(Path::new(
            "/var/mobile/Containers/Data/Application/UUID/Library/UberEats/store.db"
        )));
