use strata_core::parser::{ArtifactParser, ParsedArtifact, ParserError};
use serde::{Deserialize, Serialize};
use std::path::Path;

pub struct MalwareParser;

impl MalwareParser {
    pub fn new() -> Self {
        Self
    }
}

#[derive(Debug, Serialize, Deserialize)]
pub struct RansomwareIndicator {
    pub file_path: Option<String>,
    pub ransomware_family: Option<String>,
    pub encrypted_file_extensions: Vec<String>,
    pub ransom_note_name: Option<String>,
    pub encryption_algorithm: Option<String>,
    pub decryption_key_found: bool,
    pub confidence_score: f32,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct SuspiciousProcess {
    pub process_name: Option<String>,
    pub pid: Option<i32>,
    pub parent_pid: Option<i32>,
    pub command_line: Option<String>,
    pub file_path: Option<String>,
    pub hash_sha256: Option<String>,
    pub suspicious_behavior: Vec<String>,
    pub reputation: Option<String>,
    pub network_connections: Vec<String>,
    pub created_at: Option<i64>,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct MalwareSignature {
    pub signature_name: Option<String>,
    pub malware_family: Option<String>,
    pub file_path: Option<String>,
    pub hash_md5: Option<String>,
    pub hash_sha1: Option<String>,
    pub hash_sha256: Option<String>,
    pub detection_rule: Option<String>,
    pub yara_matches: Vec<String>,
    pub confidence: f32,
}

#[derive(Debug, Serialize, Deserialize)]
pub struct C2Beacon {
    pub process_name: Option<String>,
    pub source_ip: Option<String>,
    pub destination_ip: Option<String>,
    pub destination_port: Option<u16>,
    pub protocol: Option<String>,
    pub beacon_interval_seconds: Option<i32>,
    pub jitter_percent: Option<i32>,
    pub first_seen: Option<i64>,
    pub last_seen: Option<i64>,
    pub total_connections: i32,
}

impl Default for MalwareParser {
    fn default() -> Self {
        Self::new()
    }
}

impl ArtifactParser for MalwareParser {
    fn name(&self) -> &str {
        "Malware Detection"
    }

    fn artifact_type(&self) -> &str {
        "malware"
    }

    fn target_patterns(&self) -> Vec<&str> {
        vec!["malware", "ransomware", "c2", "beacon"]
    }

    fn parse_file(
        &self,
        path: &Path,
        data: &[u8],
    ) -> Result<Vec<ParsedArtifact>, ParserError> {
        let mut artifacts = Vec::new();

        if data.len() > 0 {
            let entry = SuspiciousProcess {
                process_name: path.file_name()
                    .map(|n| n.to_string_lossy().to_string()),
                pid: None,
                parent_pid: None,
                command_line: None,
                file_path: Some(path.to_string_lossy().to_string()),
                hash_sha256: None,
                suspicious_behavior: vec![],
                reputation: None,
                network_connections: vec![],
                created_at: None,
            };

            artifacts.push(ParsedArtifact {
                timestamp: None,
                artifact_type: "malware".to_string(),
                description: "Malware indicator".to_string(),
                source_path: path.to_string_lossy().to_string(),
                json_data: serde_json::to_value(&entry).unwrap_or_default(),
            });
        }

        Ok(artifacts)
    }
}
