...
fn parse_keystore_aliases(
    ...
    let alias_re: Regex =
        match Regex::new(r"(?:alias|name|key_alias)\s*[:=]\s*([A-Za-z0-9._:-]{3,128})") {
            Ok(r) => r,
            Err(_) => return,
        };
...
fn parse_wifi_config_store(path: &Path, text: &str, out: &mut Vec<ParsedArtifact>) {
    ...
    let ssid_re: Regex = match Regex::new(r#"<string name="SSID">"?([^"<]+)"?</string>"#) {
        Ok(r) => r,
        Err(_) => return,
    };
    let psk_re: Regex = match Regex::new(r#"<string name="PreSharedKey">"?([^"<]+)"?</string>"#) {
        Ok(r) => r,
        Err(_) => return,
    };
    ...
    let ssids: Vec<String> = ssid_re
        .captures_iter(text)
        .filter_map(|c: Captures<'_>| c.get(1).map(|m| m.as_str().trim_matches('"').to_string()))
        .collect();
