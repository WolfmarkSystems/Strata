...
use std::io::Read;
use std::path::Path;
use strata_core::parser::{ParsedArtifact, ParserError};
...
fn map_fsevent_flags(f: u32) -> Vec<String> {
    ...
    if f & 0x00000001 != 0 {
        flags.push("MustScanSubDirs".to_string());
    }
    if f & 0x00000002 != 0 {
        flags.push("UserDropped".to_string());
    }
    if f & 0x00000004 != 0 {
        flags.push("KernelDropped".to_string());
    }
    if f & 0x00000008 != 0 {
        flags.push("EventIdsWrapped".to_string());
    }
    if f & 0x00000010 != 0 {
        flags.push("HistoryDone".to_string());
    }
    if f & 0x00000020 != 0 {
        flags.push("RootChanged".to_string());
    }
    if f & 0x00000040 != 0 {
        flags.push("Mount".to_string());
    }
    if f & 0x00000080 != 0 {
        flags.push("Unmount".to_string());
    }
    if f & 0x00000100 != 0 {
        flags.push("Created".to_string());
    }
    if f & 0x00000200 != 0 {
        flags.push("Removed".to_string());
    }
    if f & 0x00000400 != 0 {
        flags.push("InodeMetaMod".to_string());
    }
    if f & 0x00000800 != 0 {
        flags.push("Renamed".to_string());
    }
    if f & 0x00001000 != 0 {
        flags.push("Modified".to_string());
    }
    if f & 0x00002000 != 0 {
        flags.push("Exchange".to_string());
    }
    if f & 0x00004000 != 0 {
        flags.push("FinderInfoMod".to_string());
    }
    if f & 0x00008000 != 0 {
        flags.push("FolderCreated".to_string());
    }
    if f & 0x00010000 != 0 {
        flags.push("PermissionMod".to_string());
    }
    if f & 0x00020000 != 0 {
        flags.push("ExtendedAttrMod".to_string());
    }
    if f & 0x00040000 != 0 {
        flags.push("ExtendedAttrRemoved".to_string());
    }
    if f & 0x00100000 != 0 {
        flags.push("DocumentRevision".to_string());
    }
    flags
}
