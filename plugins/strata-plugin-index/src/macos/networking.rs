...
impl ArtifactParser for MacosNetworkingParser {
    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        if path_str.contains("airport.preferences") {
            if let Some(known_networks) = plist_val
                .as_dictionary()
                .and_then(|d| d.get("KnownNetworks"))
                .and_then(|v| v.as_dictionary())
            {
                for (id, network) in known_networks {
                    let ssid =
                        get_string_from_plist(network, "SSIDString").unwrap_or_else(|| id.clone());
            ...
            if let Some(devices) = plist_val
                .as_dictionary()
                .and_then(|d| d.get("DeviceCache"))
                .and_then(|v| v.as_dictionary())
            {
