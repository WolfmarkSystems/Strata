...
impl ArtifactParser for LaunchdParser {
    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        use crate::parsers::plist_utils::{parse_plist_data, get_string_from_plist, get_bool_from_plist, get_int_from_plist};
        ...
        if let Some(args) = plist_val.as_dictionary().and_then(|d| d.get("ProgramArguments")).and_then(|v| v.as_array()) {
            for arg in args {
        ...
        artifacts.push(ParsedArtifact {
            ...
            description: format!("macOS {} ({})", self.agent_type.name(), entry.label.as_deref().unwrap_or("unknown")),
