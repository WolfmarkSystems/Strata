    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        let sqlite_result = with_sqlite_connection(path, data, |conn| {
            ...
            if table_exists(conn, "HMPERSON") {
                 let mut stmt = conn.prepare("SELECT ZNAME FROM HMPERSON LIMIT 100").map_err(|e| ParserError::Database(e.to_string()))?;
                 let rows = stmt.query_map([], |row| row.get::<_, String>(0)).map_err(|e| ParserError::Database(e.to_string()))?;
