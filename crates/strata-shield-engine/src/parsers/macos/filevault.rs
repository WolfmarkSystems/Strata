...
impl ArtifactParser for MacosFileVaultParser {
    ...
    fn target_patterns(&self) -> Vec<&str> {
        vec!["encryptedroot.plist", "applecustomerrecovery.plist", "filevault"]
    }
    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        if path_str.ends_with("encryptedroot.plist") {
             if let Ok(plist_val) = parse_plist_data(data) {
                let uuid = plist_val.as_dictionary().and_then(|d| d.get("VolumeUUID")).and_then(|v| v.as_string()).unwrap_or("unknown").to_string();
                
                let info = FileVaultKeyInfo {
                    volume_uuid: uuid,
                    key_type: "FV2 Recovery Blob".to_string(),
                    recovery_key_hint: plist_val.as_dictionary().and_then(|d| d.get("RecoveryKeyHint")).and_then(|v| v.as_string()).map(|s| s.to_string()),
                ...
                artifacts.push(ParsedArtifact {
                    ...
                    description: format!("FileVault 2 Recovery Blob for Volume {}", info.volume_uuid),
