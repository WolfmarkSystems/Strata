        ...
        if path_str.ends_with("backupinfo.plist") {
             if let Ok(plist_val) = parse_plist_data(data) {
                let entry = TimeMachineEntry {
                    snapshot_id: path.parent().and_then(|p| p.file_name()).map(|n| n.to_string_lossy().to_string()),
                    backup_date: None,
                    volume_name: plist_val.as_dictionary().and_then(|d| d.get("VolumeName")).and_then(|v| v.as_string()).map(|s| s.to_string()),
                ...
                artifacts.push(ParsedArtifact {
                    ...
                    description: format!("Time Machine Backup Set ({})", entry.volume_name.as_deref().unwrap_or("unknown")),
...
fn parse_snapshot_time(token: &str) -> Option<i64> {
    let mut normalized = token.replace('.', "-").replace('_', "-").replace(':', "-");
...
pub fn reconstruct_file_history(
    ...
    for art in available_artifacts {
        if art.artifact_type == "backup" {
             if let Some(desc) = art.description.to_lowercase().contains(target_name).then(|| art.description.clone()) {
