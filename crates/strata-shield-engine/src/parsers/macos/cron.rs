    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        for line in text.lines() {
            ...
            if is_system && parts.len() >= 7 {
                ...
                let is_suspicious = cmd.contains("curl") || cmd.contains("wget") || cmd.contains("python") || cmd.contains("bash -i") || cmd.contains("nc -e");
                ...
                artifacts.push(ParsedArtifact {
                 ...
                 let is_suspicious = cmd.contains("curl") || cmd.contains("wget") || cmd.contains("python") || cmd.contains("bash -i") || cmd.contains("nc -e");
                 let description = if is_suspicious {
                    format!("[ALERT] Suspicious User Cron ({}): {}", user_name.as_deref().unwrap_or("unknown"), cmd)
                 } else {
                    format!("User Cron ({}): {}", user_name.as_deref().unwrap_or("unknown"), cmd)
                 };
