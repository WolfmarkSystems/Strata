...
impl UnifiedLogLiveAnalyzer {
    ...
    pub fn analyze_stream(&self, artifact: &ParsedArtifact) -> Option<ParsedArtifact> {
        ...
        if desc_lower.contains("login") || desc_lower.contains("authentication") || desc_lower.contains("root") {
             if desc_lower.contains("fail") || desc_lower.contains("deny") || desc_lower.contains("error") {
