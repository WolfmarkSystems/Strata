...
pub fn parse_tracev3(path: &Path, data: &[u8], out: &mut Vec<ParsedArtifact>) -> Result<(), ParserError> {
...
fn parse_log_chunk(path: &Path, _chunk_data: &[u8], out: &mut Vec<ParsedArtifact>) {
    ...
    let entry = UnifiedLogEntry {
        ...
        category: None,
        message: Some(format!("Detected binary tracev3 log chunks in: {}", path.display())),
        process: None,
        pid: None,
        tid: None,
        activity_id: None,
