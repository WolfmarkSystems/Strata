...
impl ArtifactParser for MacosRecentItemsParser {
    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        if path_str.ends_with(".plist") {
            ...
            if let Some(recent_apps) = plist_val.as_dictionary().and_then(|d| d.get("RecentApplications")).and_then(|v| v.as_dictionary()).and_then(|v| v.get("CustomListItems")).and_then(|v| v.as_array()) {
            ...
            if let Ok(plist_val) = parse_plist_data(data) {
                 if let Some(items) = plist_val.as_dictionary().and_then(|d| d.get("items")).and_then(|v| v.as_array()) {
                    for item in items {
                         let name = get_string_from_plist(item, "name").unwrap_or_else(|| "unknown".to_string());
                         artifacts.push(ParsedArtifact {
                            timestamp: None,
