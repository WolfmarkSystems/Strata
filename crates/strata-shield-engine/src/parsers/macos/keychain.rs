    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        if is_keychain_db {
            ...
            let sqlite_result = with_sqlite_connection(path, data, |conn| {
                ...
                if table_exists(conn, "genp") {
                    let mut stmt = conn.prepare(
                        "SELECT service, acct, cdate, mdate FROM genp LIMIT 10000"
                    ...
                    let rows = stmt.query_map([], |row| {
                        Ok(KeychainEntry {
                            ...
                            server: None,
                            password: None,
                            creation_date: row.get::<_, f64>(2).ok().map(|d| d as i64 + 978307200),
                            modification_date: row.get::<_, f64>(3).ok().map(|d| d as i64 + 978307200),
                    ...
                    for row in rows.flatten() {
                         entries.push(ParsedArtifact {
                            ...
                            description: format!("Keychain Entry ({}): {}", row.account.as_deref().unwrap_or("unknown"), row.service.as_deref().unwrap_or("unknown")),
            ...
            artifacts.push(ParsedArtifact {
                timestamp: None,
                artifact_type: "credential".to_string(),
                description: format!("macOS Keychain File ({})", entry.item_class.as_deref().unwrap_or("unknown")),
                source_path: path.to_string_lossy().to_string(),
