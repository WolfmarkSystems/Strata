...
impl ArtifactParser for MacosKextParser {
    ...
    fn parse_file(&self, path: &Path, data: &[u8]) -> Result<Vec<ParsedArtifact>, ParserError> {
        ...
        if (path_str.contains("/extensions/") || path_str.contains("/driverextensions/")) && path_str.ends_with("info.plist") {
            if let Ok(plist_val) = parse_plist_data(data) {
                let bundle_id = get_string_from_plist(&plist_val, "CFBundleIdentifier").unwrap_or_else(|| "unknown".to_string());
                let name = get_string_from_plist(&plist_val, "CFBundleName").unwrap_or_else(|| "unknown".to_string());
                let version = get_string_from_plist(&plist_val, "CFBundleShortVersionString").unwrap_or_else(|| "0.0".to_string());
                ...
                artifacts.push(ParsedArtifact {
                    ...
                    description: format!("Kernel Extension Identified: {} ({})", info.name, info.bundle_id),
