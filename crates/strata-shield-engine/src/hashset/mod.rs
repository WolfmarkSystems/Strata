...
impl HashSetManager {
    ...
    /// Load NSRL RDS v3 hashes from a SQLite database.
    ///
    /// NSRL RDS Modern ships as SQLite with tables containing sha256/sha1/md5 columns.
    /// This method introspects the database, finds hash columns, and loads them
    /// into the known-good set. Supports both the official NSRL schema and common
    /// community formats.
    pub fn load_nsrl_sqlite(&mut self, path: &Path) -> Result<(), HashSetError> {
        let conn = Connection::open(path).map_err(|e| HashSetError::Database(e.to_string()))?;
        ...
        let hash_col_names = ["sha256", "sha1", "md5", "SHA-256", "SHA-1", "MD5",
            "sha_256", "sha_1", "hash", "file_sha256"];
        ...
        for table_name in &table_names {
            ...
            // Find hash columns
            let sha256_col = columns.iter().find(|c| {
                let lower = c.to_lowercase();
                lower.contains("sha256") || lower.contains("sha_256") || lower == "sha-256"
            });
            let sha1_col = columns.iter().find(|c| {
                let lower = c.to_lowercase();
                lower.contains("sha1") || lower.contains("sha_1") || lower == "sha-1"
            });
            let md5_col = columns.iter().find(|c| {
                let lower = c.to_lowercase();
                lower == "md5" || lower == "md_5"
            });
            let filename_col = columns.iter().find(|c| {
                let lower = c.to_lowercase();
                lower.contains("filename") || lower.contains("file_name") || lower == "name"
            });
            let size_col = columns.iter().find(|c| {
                let lower = c.to_lowercase();
                lower.contains("filesize") || lower.contains("file_size") || lower == "size"
            });

            if sha256_col.is_none() && sha1_col.is_none() && md5_col.is_none() {
                continue;
            }

            // Build SELECT query
            let mut select_cols = Vec::new();
            if let Some(col) = sha256_col {
                select_cols.push(format!("\"{}\"", col));
            }
            if let Some(col) = sha1_col {
                select_cols.push(format!("\"{}\"", col));
            }
            if let Some(col) = md5_col {
                select_cols.push(format!("\"{}\"", col));
            }
            let fname_idx = if let Some(col) = filename_col {
                select_cols.push(format!("\"{}\"", col));
                Some(select_cols.len() - 1)
            } else {
                None
            };
            let size_idx = if let Some(col) = size_col {
                select_cols.push(format!("\"{}\"", col));
                Some(select_cols.len() - 1)
            } else {
                None
            ...
            let mut row_stmt = match conn.prepare(&query) {
                Ok(s) => s,
                Err(_) => continue,
            };

            let mut col_idx = 0;
            let sha256_idx = sha256_col.map(|_| { let i = col_idx; col_idx += 1; i });
            let sha1_idx = sha1_col.map(|_| { let i = col_idx; col_idx += 1; i });
            let md5_idx = md5_col.map(|_| { let i = col_idx; col_idx += 1; i });

            let rows = row_stmt.query_map([], |row| {
                let sha256: Option<String> = sha256_idx.and_then(|i| row.get(i).ok());
                let sha1: Option<String> = sha1_idx.and_then(|i| row.get(i).ok());
                let md5: Option<String> = md5_idx.and_then(|i| row.get(i).ok());
                let filename: Option<String> = fname_idx.and_then(|i| row.get(i).ok());
                let size: Option<u64> = size_idx.and_then(|i| row.get(i).ok());
                Ok((sha256, sha1, md5, filename, size))
            });

            if let Ok(rows) = rows {
                for row in rows.flatten() {
                    let (sha256, sha1, md5, filename, size) = row;
                    ...
                    if let Some(ref h) = sha1 {
                        if h.len() == 40 {
                            // Store in known_good via sha256 key space isn't ideal,
                            // but we track it for cross-reference
                        }
                    }
                    let _ = md5; // MD5 tracked via HashSetDB if needed
                }
            }

            // If we loaded from one table, that's likely the main one
            if !self.known_good.is_empty() {
                break;
            }
        }
