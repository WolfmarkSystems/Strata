...
impl HfsPlusFilesystem {
    ...
    pub fn open_at_offset(path: &Path, offset: u64) -> Result<Self, ForensicError> {
        ...
        let mut vh = HfsPlusVolumeHeader::default();
        vh.signature = signature;
        vh.version = u16::from_be_bytes(header[2..4].try_into().unwrap());
        vh.attributes = u32::from_be_bytes(header[4..8].try_into().unwrap());
        vh.blocksize = u32::from_be_bytes(header[40..44].try_into().unwrap());
        vh.total_blocks = u32::from_be_bytes(header[44..48].try_into().unwrap());
        ...
        for i in 0..8 {
            let offset = 16 + (i * 8);
            let start_block = u32::from_be_bytes(catalog_fork_data[offset..offset+4].try_into().unwrap());
            let block_count = u32::from_be_bytes(catalog_fork_data[offset+4..offset+8].try_into().unwrap());
            if block_count > 0 {
                extents.push(HfsPlusExtentDescriptor { start_block, block_count });
            }
        }
        ...
        if !catalog_file.extents.is_empty() {
            ...
            catalog_file.node_size = u16::from_be_bytes(btree_header_rec[8..10].try_into().unwrap());
            catalog_file.root_node = u32::from_be_bytes(btree_header_rec[16..20].try_into().unwrap());
            catalog_file.first_leaf_node = u32::from_be_bytes(btree_header_rec[24..28].try_into().unwrap());
            catalog_file.last_leaf_node = u32::from_be_bytes(btree_header_rec[28..32].try_into().unwrap());
        }
        Ok(Self {
            file,
            volume_header: vh,
