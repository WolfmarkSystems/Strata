...
impl ApfsReader {
    ...
    fn read_superblock(file: &mut File, offset: u64) -> Result<ApfsSuperblock, ForensicError> {
        ...
        let mut sb = ApfsSuperblock::default();
        sb.magic = APFS_MAGIC;
        sb.block_size = u32::from_le_bytes(header[36..40].try_into().unwrap());
        sb.total_blocks = u64::from_le_bytes(header[40..48].try_into().unwrap());
        sb.free_blocks = u64::from_le_bytes(header[48..56].try_into().unwrap());
        ...
        for i in 0..8 {
            ...
            sb.fs_volumes[i] = u64::from_le_bytes(header[field_offset..field_offset+8].try_into().unwrap());
            if sb.fs_volumes[i] != 0 {
                sb.num_volumes += 1;
            }
        }

        if sb.block_size == 0 { sb.block_size = 4096; }
        Ok(sb)
    }
    ...
    fn read_volume_header(&self, block_index: u64) -> Result<ApfsVolume, ForensicError> {
        ...
        file.seek(SeekFrom::Start(self.base_offset + block_index * self.block_size as u64))?;
    ...
    fn scan_for_snapshots(&self, vol: &mut ApfsVolume) {
        ...
        for i in 1..200 {
            if let Ok(block) = self.read_block_at(start_block + i) {
                 if block.windows(4).any(|w| w == b"com.") && block.windows(9).any(|w| w == b".snapshot") {
    ...
    fn read_block_at(&self, block: u64) -> Result<Vec<u8>, ForensicError> {
        let mut file = &self.file;
        file.seek(SeekFrom::Start(self.base_offset + block * self.block_size as u64))?;
        let mut buffer = vec![0u8; self.block_size as usize];
        file.read_exact(&mut buffer).map_err(|e| ForensicError::Io(e))?;
        Ok(buffer)
    }
    ...
    pub fn read_btree_node(&mut self, physical_addr: u64) -> Result<ApfsBtreeNode, ForensicError> {
        ...
        for i in 0..key_count as usize {
            ...
            let key_offset = u16::from_le_bytes(block[entry_offset..entry_offset+2].try_into().unwrap());
            let val_offset = u16::from_le_bytes(block[entry_offset+2..entry_offset+4].try_into().unwrap());
            let length = u16::from_le_bytes(block[entry_offset+4..entry_offset+6].try_into().unwrap());
    ...
    pub fn enumerate_root(&mut self, volume_index: usize) -> Result<Vec<ApfsDirEntry>, ForensicError> {
        self.enumerate_directory(volume_index, 2) // Inode 2 is Root
    }

    pub fn enumerate_directory(&mut self, volume_index: usize, inode: u64) -> Result<Vec<ApfsDirEntry>, ForensicError> {
    ...
    fn heuristic_scan_for_files(&mut self, vol_offset: u64, parent_inode: u64) -> Result<Vec<ApfsDirEntry>, ForensicError> {
        ...
        for b in scan_start..scan_end {
            if let Ok(node_data) = self.read_block(b) {
                if node_data.len() < 4096 { continue; }
            ...
            if entries.len() > 200 { break; } // Cap results for VFS stability
        }
    ...
    fn extract_dirents_from_node(&self, node: &[u8], parent_inode: u64, entries: &mut Vec<ApfsDirEntry>) {
        let key_count = u32::from_le_bytes(node[52..56].try_into().unwrap()) as usize;
        let toc_offset = 56;
        
        for i in 0..key_count {
            let entry_off = toc_offset + (i * 8);
            if entry_off + 8 > node.len() { break; }
            
            let kn_off = u16::from_le_bytes(node[entry_off..entry_off+2].try_into().unwrap()) as usize;
            let vn_off = u16::from_le_bytes(node[entry_off+2..entry_off+4].try_into().unwrap()) as usize;
            ...
            if key_abs + 15 < node.len() && val_abs + 8 < node.len() {
                ...
                if rec_parent == parent_inode {
                    let name_len = u16::from_le_bytes(node[key_abs+12..key_abs+14].try_into().unwrap()) as usize;
                    if name_len > 0 && name_len < 255 && key_abs + 14 + name_len <= node.len() {
                        let name = String::from_utf8_lossy(&node[key_abs+14..key_abs+14+name_len]).trim_matches('\0').to_string();
                        
                        // Dirent value contains InodeID (8 bytes) at start, followed by type (2 bytes)
                        let inode = u64::from_le_bytes(node[val_abs..val_abs+8].try_into().unwrap());
                        let item_type = u16::from_le_bytes(node[val_abs+8..val_abs+10].try_into().unwrap()) >> 12;
    ...
    pub fn read_file(&mut self, _volume_index: usize, _inode: u64, _offset: u64, _size: u64) -> Result<Vec<u8>, ForensicError> {
        // Normally accesses FS B-tree for J_EXTENT records mapped to the inode
        Ok(vec![])
    }

    pub fn carve_deleted_inodes(&mut self, volume_index: usize) -> Result<Vec<ApfsDirEntry>, ForensicError> {
        if volume_index >= self.volumes.len() {
             return Err(ForensicError::PartitionNotFound(volume_index as u32));
        }
            ...
            if let Ok(data) = self.read_block(b) {
                ...
                for i in (0..data.len() - 32).step_by(8) {
                     ...
                     if parent > 0 && parent < 1_000_000_000 {
                         let name_len = u16::from_le_bytes(data[i+12..i+14].try_into().unwrap()) as usize;
                         if name_len > 0 && name_len < 128 && i + 14 + name_len <= data.len() {
                             let name = String::from_utf8_lossy(&data[i+14..i+14+name_len]).to_string();
...
pub fn apfs_detect(path: &Path) -> Result<bool, ForensicError> {
    ...
    if file.read_exact(&mut header).is_ok() {
        ...
        if file.read_exact(&mut header).is_ok() {
            let magic = u32::from_le_bytes([header[32], header[33], header[34], header[35]]);
            if magic == APFS_MAGIC { return Ok(true); }
        }
    }
    Ok(false)
}
...
pub fn apfs_enumerate_directory(path: &Path, volume: usize, inode: u64) -> Result<Vec<ApfsDirEntry>, ForensicError> {
    let mut reader = apfs_open(path)?;
    reader.enumerate_directory(volume, inode)
}

pub fn apfs_read_file(path: &Path, volume: usize, inode: u64, offset: u64, size: u64) -> Result<Vec<u8>, ForensicError> {
    let mut reader = apfs_open(path)?;
