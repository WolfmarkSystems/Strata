...
impl CorrelationEngine {
    pub fn new(threshold: i64) -> Self {
        Self { time_threshold_secs: threshold }
    }

    pub fn correlate(&self, artifacts: &[ParsedArtifact]) -> Vec<CorrelatedEvent> {
        let mut correlated = Vec::new();
        let mut artifacts_with_time: Vec<&ParsedArtifact> = artifacts.iter()
            .filter(|a| a.timestamp.is_some())
            .collect();
        ...
        for (i, art) in artifacts_with_time.iter().enumerate() {
            ...
            for j in i + 1..artifacts_with_time.iter().len() {
                let next_art = artifacts_with_time[j];
...
pub fn build_link_graph(_artifacts: &[ParsedArtifact]) -> Result<Vec<CorrelatedEvent>, ForensicError> {
