...
impl EvidenceGapAnalyzer {
    ...
    pub fn analyze(charge: &ChargeEntry, outputs: &[PluginOutput]) -> Vec<EvidenceGap> {
        ...
        if section.starts_with("2252")
            || section.starts_with("2256")
            || category.contains("csam")
            || category.contains("child")
        {
            ...
            if all_text.contains("csam")
                && !all_text.contains("media")
                && !all_text.contains("image")
            {
                gaps.push(EvidenceGap {
                    charge: charge.clone(),
            ...
            if all_text.contains("onedrive")
                || all_text.contains("icloud")
                || all_text.contains("google drive")
            {
                gaps.push(EvidenceGap {
                    charge: charge.clone(),
                    missing_artifact_type: "Cloud storage contents".to_string(),
                    why_expected: "Cloud sync artifacts detected but cloud contents not acquired"
                        .to_string(),
                    investigative_recommendation:
                        "Cloud storage sync detected. Request warrant for cloud account contents."
                            .to_string(),
                });
            }
        }
        ...
        if section.starts_with("1030") || category.contains("computer") {
            if !all_text.contains("evtx")
                && !all_text.contains("event log")
                && !all_text.contains("logon")
            {
                gaps.push(EvidenceGap {
                    charge: charge.clone(),
            ...
            if !all_text.contains("network") && !all_text.contains("remote") {
                gaps.push(EvidenceGap {
                    ...
                    why_expected:
                        "CFAA requires proof of unauthorized access to protected computer"
                            .to_string(),
                    investigative_recommendation:
                        "Check RDP history, VPN logs, SSH known_hosts, network adapter history"
                            .to_string(),
                });
            }
        }
        ...
        if section == "120" || category.contains("sexual assault") {
            if !all_text.contains("message")
                && !all_text.contains("sms")
                && !all_text.contains("chat")
            {
                gaps.push(EvidenceGap {
                    charge: charge.clone(),
        ...
        if (section == "1519" || category.contains("obstruction"))
            && !all_text.contains("usn journal")
            && !all_text.contains("$usnjrnl")
        {
            gaps.push(EvidenceGap {
                    charge: charge.clone(),
