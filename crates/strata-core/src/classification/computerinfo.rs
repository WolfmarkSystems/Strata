...
fn apply_computer_info_from_reg(path: &Path, info: &mut ComputerInfo) {
    ...
    for record in &records {
        ...
        if info.name.is_empty() && lower.contains("\\control\\computername\\computername") {
        ...
        if info.domain.is_empty() && lower.contains("\\services\\tcpip\\parameters") {
            if let Some(value) = record
                .values
                .get("Domain")
        ...
        if info.install_date.is_none() && lower.contains("microsoft\\windows nt\\currentversion") {
