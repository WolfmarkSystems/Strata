...
pub fn get_schannel_cipher() -> Vec<CipherSuite> {
    let Some(items) = load(path("FORENSIC_SCHANNEL_CIPHERS", "schannel_ciphers.json")) else {
        return Vec::new();
    };
    items
        .into_iter()
        .map(|v| CipherSuite {
            name: s(&v, &["name", "cipher", "cipher_suite"]),
            enabled: b(&v, &["enabled"]),
        })
