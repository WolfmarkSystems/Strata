...
impl TagManager {
    ...
    pub fn get_tag_counts(&self) -> Result<HashMap<String, usize>, String> {
        ...
        for (name, cnt) in counts.flatten() {
...
impl EvidenceAnalyzer {
    ...
    pub fn load_plugins(&mut self, plugin_dir: &Path) -> Result<usize, String> {
        let manager = crate::plugin::PluginManager::new(plugin_dir);
    ...
    pub fn analyze(
        ...
        if let Some(ref plugin_manager) = self.plugin_manager {
            ...
            for artifact in plugin_artifacts {
                let artifact: crate::parser::ParsedArtifact = artifact.into();
            ...
            match parser.analyze_volume(vfs, vol) {
                Ok(metadata) => {
                    ...
                    for te in &timeline_entries {
                        let core_te = crate::timeline::TimelineEntry::from(te.clone());
                        if let Ok(entry_id) = self.timeline_manager.insert_entry(&core_te) {
                            total_entries += 1;
                            ...
                            event_bus.emit_simple(
                                ...
                                EngineEventKind::TimelineEntryAdded {
                                    entry_id: entry_id.to_string(),
                                    artifact_type: core_te.artifact_type.clone(),
                                },
                                EventSeverity::Info,
